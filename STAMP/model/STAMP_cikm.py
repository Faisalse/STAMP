...
import time
from STAMP.basic_layer.NN_adam import NN
from STAMP.util.Printer import TIPrint
from STAMP.util.batcher.equal_len.batcher_p import batcher
from STAMP.util.AccCalculater import cau_recall_mrr_org
from STAMP.util.AccCalculater import cau_samples_recall_mrr
from STAMP.util.Pooler import pooler
from STAMP.basic_layer.FwNn3AttLayer import FwNnAttLayer
from STAMP.util.FileDumpLoad import dump_file
...
class Seq2SeqAttNN(NN):
    ...
    def build_model(self):
        ...
        inputs = tf.nn.embedding_lookup(self.embe_dict, self.inputs,max_norm=1.5)
        lastinputs= tf.nn.embedding_lookup(self.embe_dict, self.last_inputs,max_norm=1.5)
    ...
    def train(self,sess, train_data, test_data=None,saver = None):
    ...
    def test(self,sess,test_data):
        ...
        while bt.has_next():    # batch round.
            ...
            batch_lenth = len(batch_data['in_idxes'])
            event = len(batch_data['in_idxes'][0])
            if batch_lenth > batchSize:
                patch_len = int(batch_lenth / batchSize)
                remain = int(batch_lenth % batchSize)
                i = 0
                for x in range(patch_len):
                    users+=1
                    tmp_in_data = batch_data['in_idxes'][i:i+batchSize]
                    tmp_out_data = batch_data['out_idxes'][i:i+batchSize]
                    tmp_batch_ids = batch_data['batch_ids'][i:i+batchSize]
                    for s in range(len(tmp_in_data[0])):
                        batch_in = []
                        batch_out = []
                        batch_last = []
