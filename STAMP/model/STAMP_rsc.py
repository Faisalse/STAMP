...
class Seq2SeqAttNN(NN):
    ...
    def test(self,sess,test_data):
        ...
        while bt.has_next():    # batch round.
            ...
            batch_lenth = len(batch_data['in_idxes'])
            event = len(batch_data['in_idxes'][0])
            if batch_lenth > batchSizePre:
                patch_len = int(batch_lenth / batchSizePre)
                remain = int(batch_lenth % batchSizePre)
                i = 0
                for x in range(patch_len):
                    users +=1
                    tmp_in_data = batch_data['in_idxes'][i:i+batchSizePre]
                    tmp_out_data = batch_data['out_idxes'][i:i+batchSizePre]
                    tmp_batch_ids = batch_data['batch_ids'][i:i+batchSizePre]
                    for s in range(len(tmp_in_data[0])):
                        batch_in = []
                        batch_out = []
                        batch_last = []
